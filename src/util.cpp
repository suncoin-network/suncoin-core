...
const char * const BITCOIN_CONF_FILENAME = "suncoin.conf";
const char * const BITCOIN_PID_FILENAME = "suncoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "suncoin" is a composite category enabling all SunCoin-related debug output
            if(ptrCategory->count(string("suncoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SunCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SunCoinCore
    // Mac: ~/Library/Application Support/SunCoinCore
    // Unix: ~/.suncoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SunCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SunCoinCore";
#else
    // Unix
    return pathRet / ".suncoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty suncoin.conf if it does not excist
